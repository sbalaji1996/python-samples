...
def eval_all(expressions, env):
    ...
    leng = len(expressions)
    for item in range(0, leng - 1):
        scheme_eval(expressions[item], env)
    num = expressions[leng - 1]
    return scheme_eval(num, env)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        "*** YOUR CODE HERE ***"
        target = expressions[0][0]

        if scheme_symbolp(target):
            env.define(target, LambdaProcedure(expressions[0].second, expressions.second, env))
        else:
            raise SchemeError
...
def do_and_form(expressions, env):
    ...
    for i in range(0, len_e - 1):
        temp = scheme_eval(expressions[i], env)
        if scheme_false(temp):
            return False

    return scheme_eval(expressions[len_e - 1], env)
