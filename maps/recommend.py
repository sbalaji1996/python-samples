...
def group_by_centroid(restaurants, centroids):
    ...
    """
    "*** YOUR CODE HERE ***"
    points = [[find_closest(restaurant_location(x), centroids), x] for x in restaurants]
    return group_by_first(points)
...
def find_centroid(restaurants):
    ...
    for restaurant in restaurants:
        avg_lat += restaurant_location(restaurant)[0]
        avg_long += restaurant_location(restaurant)[1]
        i += 1
    return [avg_lat/i, avg_long/i]
...
def best_predictor(user, restaurants, feature_fns):
    ...
    "*** YOUR CODE HERE ***"
    lst = [find_predictor(user, reviewed, a) for a in feature_fns]
...
def rate_all(user, restaurants, feature_functions):
    ...
    dic_of_user_rated = user_reviewed_restaurants(user, restaurants)
    lst_of_user_rated = [dic_of_user_rated.keys()]

    def get_rating(user, restaurants, feature_functions):
        for restaurant in restaurants:
            if restaurant in lst_of_user_rated:
                return user_rating(user, 'restaurant_name(restaurant)')
            else:
                return predictor

    return {'restaurant_name(elm)': get_rating(user, RESTAURANTS, feature_functions) for elm in RESTAURANTS}
...
def search(query, restaurants):
    ...
    return [[restaurant_name(restaurant)] for restaurant in restaurants if query in 'restaurant_categories(restaurant)']
